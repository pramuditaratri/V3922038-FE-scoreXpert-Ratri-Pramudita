...
Future<void> getAnggotaData() async {
  try {
    List<dynamic> data = await anggotaService.getData(); // Gunakan anggotaService
    setState(() {
  ...
  itemBuilder: (context, index) {
    ...
    String noKtp = 'No KTP tidak ditemukan';

    // Temukan no_ktp anggota dari _anggotaList
    for (var anggota in _anggotaList) {
      if (anggota.id == pengajuan.anggotaId) {
        noKtp = anggota.noKtp;  // Ambil no_ktp
  ...
  children: [
    ...
    Text('Beban Pengeluaran Bulanan: ${pengajuan.bebanPengeluaranBulanan}'),
    Text('Agunan: ${pengajuan.agunan}'),
    Text('Jenis Kredit: ${pengajuan.besaranKredit}'),
    Text('Jumlah Hutang: ${pengajuan.jumlahHutang}'),
    Text('Tanggal Pengajuan: ${pengajuan.tanggalPengajuan}'),
    Text('Tanggal Mulai: ${pengajuan.tanggalMulai}'),
    Text('Tanggal Selesai: ${pengajuan.tanggalSelesai}'),
  ],
),
                            trailing: Row(
